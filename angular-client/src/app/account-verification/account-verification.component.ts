...
export class AccountVerificationComponent implements OnInit {
  ...
  checkVerify() {
    this.http.get(`${this.API}/user/`+ this.user._id).subscribe(data => {
      this.user = data;
      if (data['isVerified']) {
        alert("Account has been verified. Please login.");
      }
      else {
        alert("Account has not been verified. Please follow the directions provided in the email to complete your signup process.");
      }
    })
  }
}

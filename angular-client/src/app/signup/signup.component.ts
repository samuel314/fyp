...
export class SignupComponent implements OnInit {
  ...
  signup(gender,admitType,local) {
    this.error = false;
    this.credentials['gender'] = gender
    this.credentials['admitType'] = admitType;
    this.credentials['local'] = local;
    this.http.post(`${this.API}/signup`, this.credentials).subscribe(res => {
      ...
      if (err.error.hasOwnProperty("errors")) {
        ...
        for (var i in err.error) {
          if (err.error[i].msg != undefined) {
            this.errMessages[i] = err.error[i].msg;
          }
        }
      }
    });

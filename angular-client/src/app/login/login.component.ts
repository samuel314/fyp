...
export class LoginComponent implements OnInit {
  ...
  signin() {
    this.http.post(`${this.API}/login`, this.credentials).subscribe(res => {
      this.http.get(`${this.API}/courseHistory/` + res['_id']).subscribe(data => {
        this.courseHistory = data;
        if (data[0]=== undefined) {
          this.router.navigate(['/sis-login']);
        }
      });
      ...
      if (err.error.hasOwnProperty("errors")) {
        for (var i in err.error.errors) {
          if (err.error.errors[i].msg != undefined) {
            this.errMessages[i] = err.error.errors[i].msg +'.';
          }
        }
      } else {
        if (err.error.msg != undefined) {
          this.errMessages["msg"] = err.error.msg;
        }
      }
    });
  }
